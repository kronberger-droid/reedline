...
impl PromptEditMode {
    ...
    pub(crate) fn selection_extent(&self) -> SelectionExtent {
        match self {
            ...
            // Helix is gap-indexed (`w` selects "foo "); the bar modes never form
            // a block selection, and `op_end` is already exclusive there, so `Span`
            // is the natural (and only sensible) reading for them too.
